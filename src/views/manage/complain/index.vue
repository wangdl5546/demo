  ...
  <div class="app-container">
    ...
    <el-row :gutter="30">
      ...
      <el-col>
        ...
        <el-table v-loading="loading" :data="userList">
          ...
          <el-table-column label="投诉时间" align="center" prop="createTime" v-if="columns[6].visible" width="240">
            <template v-slot="scope">
              <span>{{ parseTime(scope.row.createTime) }}</span>
            </template>
          </el-table-column>
          <el-table-column label="投诉内容" align="center" key="nickname" prop="nickname" v-if="columns[1].visible" :show-overflow-tooltip="true"/>
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="投诉内容" prop="nickname">
              <el-input v-model="form.nickname" placeholder="请输入投诉内容" />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
  ...
  methods: {
    ...
    getList() {
      ...
      listUser(this.queryParams).then(response => {
          ...
          response.data.list[0].nickname = '蔬菜不新鲜'

          response.data.list[1].username = '东区B2'
          response.data.list[1].nickname = '水果种类不多'

          response.data.list[2].username = '南区A1'
          response.data.list[2].nickname = '淡水水产有死鱼'

          response.data.list[3].username = '南区B1'
          response.data.list[3].nickname = '海鲜不新鲜'

          response.data.list[4].username = '西区A1'
          response.data.list[4].nickname = '肉类种类不好'

          response.data.list[5].username = '西区C1'
          response.data.list[5].nickname = '家禽分类有误'

          response.data.list[6].username = '北区A1'
          response.data.list[6].nickname = '豆制品不新鲜'

          response.data.list[7].username = '北区A2'
          response.data.list[7].nickname = '干货过期'

          response.data.list[8].username = '北区D1'
          response.data.list[8].nickname = '炒货种类不多'
