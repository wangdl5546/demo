  ...
  <div id="devicePosition" style="width: 100vw; height: 91vh;">
    <el-container v-if="onOff" style="height: 91vh;" v-loading="isLoging">
      <el-aside width="auto" style="background-color: #ffffff">
        <el-tree :data="deviceTree" :props="defaultProps" @node-click="onNodeClick" default-expand-all ref="tree">
        </el-tree>
      </el-aside>
    ...
    <div ref="infobox" v-if="channel != null">
      <div v-if="channel != null" class="infobox-content">
        <el-descriptions class="margin-top" :title="channel.name" :column="1" :colon="true" size="mini"
          ...
          <el-descriptions-item label="安装地址">{{
            channel.address == null ? '未知' : channel.address
        ...
        <div style="padding-top: 10px">
          <el-button v-bind:disabled="device == null || device.online === 0" type="primary" size="small" title="播放"
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="60%" :before-close="handleClose" destroy-on-close="true">
      <div style="width: 100%;height: 90%;display: flex;flex-wrap: wrap;background-color: #000;">
        <player v-if="dialogVisible" ref="player" :videoUrl="videoUrl" fluent autoplay></player>
      </div>
...
import player  from '../media/component/jessibuca.vue'
...
export default {
  ...
  methods: {
    initDevice: function () {
      ...
      let tree = this.$data.deviceTree;
      let points = [];
      tree.forEach(node => {
        this.foreachNode(node, n => {
          if (n.type == 'dev') {
            this.showPoint(n.lng, n.lat, n)
          }
        })
      });
    ...
    showPoint: function (lng, lat, data) {
    ...
    onNodeClick: function (data, node, component) {
      this.initDevice()
      if (data.type == 'dev') {
        this.$refs.map.panTo([data.lng, data.lat], 18)
      }
    },
