  ...
  <div class="app-container">
      ...
      <el-col>
        <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
          <el-form-item label="巡查人员" prop="username">
            <el-input v-model="queryParams.username" placeholder="巡查人员" clearable style="width: 240px"
        ...
        <el-table v-loading="loading" :data="userList">
          ...
          <el-table-column label="巡查时间" align="center" prop="createTime" v-if="columns[6].visible" width="240">
            <template v-slot="scope">
              <span>{{ parseTime(scope.row.createTime) }}</span>
            </template>
          </el-table-column>
          <el-table-column label="巡查人员" align="center" key="remark" prop="remark" v-if="columns[1].visible" :show-overflow-tooltip="true"/>
          <el-table-column label="巡查内容" align="center" key="nickname" prop="nickname" v-if="columns[1].visible" :show-overflow-tooltip="true"/>
          <el-table-column label="巡查地点" align="center" key="username" prop="username" v-if="columns[1].visible" :show-overflow-tooltip="true"/>
          <el-table-column label="巡查结果" align="center" key="email" prop="email" v-if="columns[1].visible" :show-overflow-tooltip="true"/>
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-row>
          <el-col :span="12">
            <el-form-item label="巡查人员" prop="remark">
              <el-input v-model="form.remark" placeholder="请输入巡查人员" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item label="巡查内容" prop="nickname">
              <el-input v-model="form.nickname" placeholder="请输入巡查内容" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item label="巡查地点" prop="username">
              <el-input v-model="form.username" placeholder="请输入巡查地点" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item label="巡查结果" prop="email">
              <el-input v-model="form.email" placeholder="请输入巡查结果" />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
  ...
  methods: {
    ...
    getList() {
      ...
      listUser(this.queryParams).then(response => {
          ...
          response.data.list[0].email = '正常'
          response.data.list[0].nickname = '蔬菜区消防巡逻'
          response.data.list[0].remark = '王五'

          response.data.list[1].username = '东区B2'
          response.data.list[1].email = '正常'
          response.data.list[1].nickname = '水果区消防巡逻'
          response.data.list[1].remark = '赵六'

          response.data.list[2].username = '南区A1'
          response.data.list[2].email = '正常'
          response.data.list[2].nickname = '淡水水产区消防巡逻'
          response.data.list[2].remark = '犀利'

          response.data.list[3].username = '南区B1'
          response.data.list[3].email = '正常'
          response.data.list[3].nickname = '海鲜区消防巡逻'
          response.data.list[3].remark = '梓潼'

          response.data.list[4].username = '西区A1'
          response.data.list[4].email = '正常'
          response.data.list[4].nickname = '肉类区消防巡逻'
          response.data.list[4].remark = '一辉'

          response.data.list[5].username = '西区C1'
          response.data.list[5].email = '正常'
          response.data.list[5].nickname = '家禽区消防巡逻'
          response.data.list[5].remark = '星耀'

          response.data.list[6].username = '北区A1'
          response.data.list[6].email = '正常'
          response.data.list[6].nickname = '豆制品区消防巡逻'
          response.data.list[6].remark = '小A'

          response.data.list[7].username = '北区A2'
          response.data.list[7].email = '正常'
          response.data.list[7].nickname = '干货区消防巡逻'
          response.data.list[7].remark = '菊琴'

          response.data.list[8].username = '北区D1'
          response.data.list[8].email = '正常'
          response.data.list[8].nickname = '炒货区消防巡逻'
